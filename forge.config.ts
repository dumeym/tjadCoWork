...
const apkName = 'tjadCoWork_' + packageJson.version + '_' + (process.env.arch || process.arch);
...
module.exports = {
  packagerConfig: {
    ...
    executableName: 'tjadCoWork',
    out: path.resolve(__dirname, 'out'),
    tmpdir: path.resolve(__dirname, '../tjadCoWork-tmp'),
    extraResource: [path.resolve(__dirname, 'public')],
    win32metadata: {
      CompanyName: 'tjadcowork',
      FileDescription: 'TJAD AI Agent Desktop Interface',
      OriginalFilename: 'tjadCoWork.exe', // 简化文件名
      ProductName: 'tjadCoWork',
      InternalName: 'tjadCoWork',
  ...
  makers: [
    ...
    ...(MakerSquirrel
      ? [
          new MakerSquirrel(
            {
              name: 'tjadCoWork', // 必须与 package.json 的 name 一致
    ...
    new MakerWix(
      {
        name: 'tjadCoWork',
        description: 'TJAD AI Agent Desktop Interface',
        exe: 'tjadCoWork',
